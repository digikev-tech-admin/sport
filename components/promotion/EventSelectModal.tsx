...
const EventSelectModal: React.FC<EventSelectModalProps> = ({ open, onOpenChange, events, selectedIds, setSelectedIds }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!w-[80vw]  !max-w-[80vw] rounded-xl p-6 top-[5vh] translate-y-0 max-h-[90vh] overflow-auto">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold">Select Events</DialogTitle>
        </DialogHeader>
          ...
          {selectedIds.length > 0 && (
            <div className="text-sm">
              ...
              {events.filter(e => selectedIds.includes(e.id)).map(e => (
                <span key={e.id} className="mr-2">{e.title}</span>
              ))}
            </div>
          )}
                    ...
                    <TableRow key={ev.id} className={(isSelected ? 'bg-[#7421930A] ' : '') + 'hover:bg-accent/40'}>
                      ...
                      <TableCell className="text-right">
                        <Button
                          ...
                          variant="ghost"
                          onClick={() => {
                            setSelectedIds(isSelected ? selectedIds.filter(id => id !== ev.id) : [...selectedIds, ev.id])
                          }}
                        >
