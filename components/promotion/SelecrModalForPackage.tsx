...
const PackageSelectModal: React.FC<PackageSelectModalProps> = ({ open, onOpenChange, packages, initialId, onSave }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!w-[80vw]  !max-w-[80vw] rounded-xl p-6 top-[5vh] translate-y-0 max-h-[90vh] overflow-auto">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold">Select Package</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid gap-4">
          ...
          {localSelectedId && (
            <div className="text-sm">
              ...
              <span>{packages.find(p => p.id === localSelectedId)?.title || ''}</span>
            </div>
          )}
            ...
            <Table>
              ...
              <TableBody>
                {pageItems.map((pkg, idx) => {
                  const isSelected = localSelectedId === pkg.id;
                  return (
                    <TableRow key={pkg.id} className={(isSelected ? 'bg-[#7421930A] ' : '') + 'hover:bg-accent/40'}>
                      ...
                      <TableCell className="text-right">
                        <Button
                          ...
                          variant="ghost"
                          onClick={() => {
                            setLocalSelectedId(isSelected ? '' : pkg.id)
                          }}
                        >
